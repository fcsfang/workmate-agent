                    ...
                    if res.returncode == 0:
                        ...
                        if image_base64_list:
                            ...
                            prompt = (
                                ...
                                "2. 防范『元工具拖延』：如果用户目前正在调试、开发、查阅或配置当前这个 Workmate Agent 项目本身（窗口标题、页面内容或文件路径中通常包含 `workmate-agent`、`antigravity`、`workmate` 等），除非用户的专注目标明确写着与『开发/调试 Workmate Agent 本身』直接相关，否则这属于典型的拖延和偏离目标行为，必须判定为【偏航】（偏航原因指出：『用户正在阅读/调试本监督系统/Agent项目，而非推进设定的目标 {goal}』）。\n"
